...
namespace pharmacufrom
{
    public partial class WarehouseStaffdrugs : Form
    {
        ...
        private void WarehouseStaffdrugs_Load(object sender, EventArgs e)
        {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                int stockId = int.Parse(textBoxStockId.Text);
                int result = soapClient.DeleteStockWeb(stockId);

                MessageBox.Show(result > 0 ? "Stock deleted successfully." : "Failed to delete stock.");
                BindData();
            }
            catch (Exception ex)
            {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)  // Ensure a valid row is clicked
            {
                ...
                textBoxStockId.Text = row.Cells["StockId"].Value.ToString();
            ...
            try
            {
                using (SqlConnection con = new SqlConnection("Server=LAPTOP-O6BJ19N8\\SQLEXPRESS02;Database=spcdb;Trusted_Connection=True;"))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, con))
                    {
                        ...
                        command.Parameters.AddWithValue("@StockId", string.IsNullOrWhiteSpace(textBoxStockId.Text) ? 0 : int.Parse(textBoxStockId.Text));  // If StockId is empty, use 0
        ...
        private void textBoxDrugName1_TextChanged(object sender, EventArgs e)
        {
